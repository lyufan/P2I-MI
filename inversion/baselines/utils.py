...
def init_align_dataloader(file_path, batch_size=64, drop_last=True):
    tf = time.time()
    data_set = dataloader.AlignFolder(file_path)
    data_loader = torch.utils.data.DataLoader(data_set,
                                                  batch_size=batch_size,
                                                  shuffle=True,
                                                  drop_last=drop_last,
...
def high2low(img):
    # 0 and 1, 64 to 112
    bs = img.size(0)
    proc = get_deprocessor2()  # Resize to 112*112, and toTensor
...
def calc_knn(feat, iden, dataset, path="feature"):
    iden = iden.cpu().long()
    feat = feat.cpu()
    true_feat = torch.from_numpy(np.load(os.path.join(path, "feat_{}.npy".format(dataset)))).float()
    info = torch.from_numpy(np.load(os.path.join(path, "info_{}.npy".format(dataset)))).view(-1).long()
    bs = feat.size(0)
    ...
    for i in range(bs):
        ...
        for j in range(tot):
            if info[j] == iden[i]:
                # print(true_feat[j, :].shape)
                dist = torch.sum((feat[i, :] - true_feat[j, :]) ** 2)
                knn = min(knn, dist)
        knn_dist += knn
    return (knn_dist / bs).item()

def calc_cos(feat, iden, dataset, path="feature"):
    ...
    for i in range(bs):
        ...
        for j in range(tot):
            if info[j] == iden[i]:
                # print(true_feat[j, :].shape)
                dist = cos_sim(feat[i, :], true_feat[j, :])
                # dist = torch.cosine_similarity(feat[i, :],true_feat[j, :], dim=0)
                knn_dist += dist
                m += 1
        knn_dist = knn_dist/m
        cos_dist += knn_dist

    return (cos_dist / bs).item()

def calc_feat_dist(feat, iden, dataset, path="feature"):
    iden = iden.cpu().long()
    feat = feat.cpu()
    true_feat = torch.from_numpy(np.load(os.path.join(path, "feat_{}.npy".format(dataset)))).float()
    info = torch.from_numpy(np.load(os.path.join(path, "info_{}.npy".format(dataset)))).view(-1).long()
    ...
    for i in range(bs):
        ...
        for j in range(tot):
            if info[j] == iden[i]:
                id_feat.append(true_feat[j, :])
        centroid = torch.mean(torch.stack(id_feat, dim=0), dim=0)
        dist = torch.sum((feat[i, :] - centroid.squeeze(0)) ** 2)
        feat_dist += dist
    return (feat_dist / bs).item()
