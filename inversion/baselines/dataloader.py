...
class ImageFolder(data.Dataset):
    ...
    def get_list(self, file_path):
        ...
        # for root, _, fnames in sorted(os.walk(self.img_path)):
        #     for fname in fnames:
        #         # path = os.path.join(root, fname)
        #         path = fname
        #         name_list.append(path)
        #         label_list.append(int(os.path.basename(fname).strip().split('_')[0]))
        # return name_list, label_list
    ...
    def load_img(self):
        ...
        for i, img_name in enumerate(self.name_list):
            if img_name.endswith(".png") or img_name.endswith(".jpg") or img_name.endswith(".jpeg"):
                ...
                # path = self.img_path + "/" + img_name.strip().split('.')[0] + "_0.png"
                # path = img_name
    ...
    def get_processor(self):
        ...
        if self.mode == "train":
            ...
            proc.append(transforms.ToTensor())
            if self.mode == 'test' or self.mode == 'train' or self.mode == 'gan' or self.args["dataset"]["name"] != 'facescrub':
                proc.append(transforms.Lambda(crop))
            proc.append(transforms.ToPILImage())
            proc.append(transforms.Resize((re_size, re_size)))
            proc.append(transforms.ToTensor())
    ...
    def __getitem__(self, index):
        ...
        if self.mode == 'style':
            # utils.save_tensor_images(img, os.path.join("~/P2I_MI/inversion/data_files/test", name))
...
class AlignFolder(data.Dataset):
    def __init__(self, file_path):
        self.img_path = "~/plg/datasets/ffhq10000"
        self.model_name = "VGG16"
        # self.img_list = os.listdir(self.img_path)
        self.processor = self.get_processor()
        self.name_list = self.get_list(file_path)
        self.image_list = self.load_img()
        self.latent_list = self.load_latent()
        self.num_img = len(self.image_list)
        print("Load " + str(self.num_img) + " images")
    ...
    def load_latent(self):
        latent_list = []
        for i, img_name in enumerate(self.name_list):
            path = "~/trans/data_files/latent/latent_code_" + str(int((img_name.strip().split('.'))[0])) + ".npy"
            latent = np.load(path)
