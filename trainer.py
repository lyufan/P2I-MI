...
class Trainer(nn.Module):
    def __init__(self, config, opts):
        ...
        # self.enc = fs_encoder_v3(n_styles=self.n_styles)
        # self.enc = fs_encoder_v4()
        # self.enc_fix = fs_encoder_v2_fix(n_styles=self.n_styles, opts=opts, residual=enc_residual,
    ...
    def initialize(self, stylegan_model_path, arcface_model_path, parsing_model_path):
        ...
        self.noise_inputs = [getattr(self.StyleGAN.noises, f'noise_{i}').to(self.device) for i in
    ...
    def LPIPS(self, input, target, multi_scale=False):
        ...
        if multi_scale:
            out = 0
            for k in range(3):
                # downscale(input, k, self.scale_mode)
    ...
    def get_image(self, w=None, f=None, img=None, noise=None, training_mode=True):
        ...
        if 'use_fs_encoder' in self.config and self.config['use_fs_encoder']:
            ...
            w_recon, fea, outimg = self.enc(x_1)
            w_recon = w_recon + self.dlatent_avg
            if training_mode:
                features = None
            else:
        ...
        return [x_1_recon, x_1, w_recon, w_delta, n_1, fea, fea_recon, outimg]

    def compute_loss(self, w=None, real_img=None, img=None, noise=None):
        return self.compute_loss_stylegan2(w=w, real_img=real_img, img=img, noise=noise)

    def compute_loss_stylegan2(self, w=None, real_img=None, img=None, noise=None):
        img_ori = img
        if img is None:
        ...
        if img is not None and real_img is not None:
            ...
            img = torch.cat([img[0], real_img[0]], dim=0)
            noise = [torch.cat([nn, nn], dim=0) for nn in noise]


        out = self.get_image(w=w, img=img, noise=noise)
        x_1_recon, x_1, w_recon, w_delta, n_1, fea_1, fea_recon, outimg = out
        x_1 = torch.cat([img_ori[1], real_img[1]], dim=0)

        # Loss setting
        w_l2, w_lpips, w_id = self.config['w']['l2'], self.config['w']['lpips'], self.config['w']['id']
        b = x_1.size(0) // 2
        # b1 = x_1.size(0)
        if 'l2loss_on_all_image' in self.config and self.config['l2loss_on_all_image']:
            b = x_1.size(0)
        self.l2_loss = self.L2loss(x_1_recon[:b], x_1[:b]) + self.L2loss(outimg[:b], downscale(x_1[:b], self.scale, self.scale_mode)) if w_l2 > 0 else torch.tensor(0)  # l2 loss only on synthetic data
        # LPIPS
    ...
    def test(self, w=None, f=None, img=None, noise=None, return_latent=False, realimg=None,
             training_mode=False):
        if 'n_iter' not in self.__dict__.keys():
            self.n_iter = 1e5
        out = self.get_image(w=w, f=f, img=img, noise=noise, training_mode=True)
    ...
    def save_image_stylegan2(self, log_dir, n_epoch, n_iter, prefix='/train/', w=None, img=None, noise=None,
                             training_mode=True):
        ...
        with torch.no_grad():
            ...
            x_1_recon = downscale(x_1_recon, 2, self.scale_mode)
            ...
            if 'use_fs_encoder' in self.config and self.config['use_fs_encoder']:
                ...
                x_1_recon_2 = downscale(x_1_recon_2, 2, self.scale_mode)
    ...
    def save_model(self, log_dir, n_epoch):
        torch.save(self.enc.state_dict(), '{:s}/enc{}.pth.tar'.format(log_dir, n_epoch))
    ...
    def update(self, w=None, real_img=None, noise=None, img=None, n_iter=0):
        self.n_iter = n_iter
        self.enc_opt.zero_grad()
        self.sum_loss = self.compute_loss(w=w, real_img=real_img, img=img, noise=noise)
